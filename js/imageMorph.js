 ...
 *
 * This module recreates a directional popup pattern for opening and
 ...
 * position to the center of the viewport. Then it swaps in the real modal
 ...
 * deceleration. Duration scales with travel distance. Short trips feel
 * snappy (about 180 ms) on mobile. Long trips have enough time (about
 * 400 ms) on desktop. The feel stays consistent across every screen size.
 *
 * Per Apple Design, the morph starts immediately on pointer-down. It uses
 * the already-loaded thumbnail as the clone source. The full-resolution
 * image loads in the background and swaps in without a visible hiccup.
 * The user never waits for a network request before seeing motion.
 *
 * If the user prefers reduced motion, the modal toggles instantly without
 * any animation.
 */

import { fullResDecodeState } from "./imagePreloading.js";

const morph = (() => {
  /* modal.js passes these DOM references in during init(). */
  let modal = null;
  let modalImg = null;
  ...
  /* Keep a reference to the thumbnail that triggered the current open.
     The close animation retraces the same path even when the modal
     caller does not pass it again. */
  let closeSourceEl = null;

  /* The rotation angle (in degrees) of the polaroid card that was
     clicked. Capture it during open and apply it during close. The clone
     rotates back into the card's tilt instead of snapping flat at the
     last frame. */
  ...
  /* The translateX and translateY values from the polaroid card's CSS
     transform. Extract them during open and apply them during close.
     The clone then matches the card's full rotate()+translate()
     transform instead of just the rotation. Default to 0 for images
  ...
  /* These easing tokens follow Emil Kowalski's animation blueprint.
     ease-out-quint is faster than ease-out-quart on the initial
  ...
  /* Open and close share the same easing. The motion is symmetric.
     ...
     computeMorphDuration). Short trips feel snappy. Long trips
  ...
  /* Snapshot a DOMRect into a plain object. The values do not go stale
     when the browser repaints. */
  const freezeRect = (r) => ({
    left: r.left, top: r.top, width: r.width, height: r.height,
  });

  /* Capture layout-accurate dimensions from a live element. Correct for
     CSS rotation. getBoundingClientRect() returns the axis-aligned
     bounding box of a rotated element. That box is bigger than the
     element's true visual footprint. Use offsetWidth and offsetHeight
     (layout dimensions without transforms) for the size. Adjust the
     position so the rotation centre stays anchored. This removes the
     2-6 px double-compensation that causes jitter when the clone lands
     on the card. The mismatch is worst on small mobile screens. */
  ...
  /* Return a morph duration based on the Euclidean distance between
     two rectangles. Short trips feel snappy (floor 180 ms). Long trips
     get enough time to read as intentional (ceiling 400 ms). Distance
     is measured between the centres of the source and target rects.
     It is normalised against a typical desktop viewport diagonal
  ...
  const computeMorphDuration = (fromRect, toRect) => {
    ...
    /* Map ratio 0-1 across the 0.18s-0.40s range.
       A center-screen tap (ratio near 0) gets about 180 ms. Snappy.
  ...
  const killTweens = () => {
    /* Restore the polaroid card that a close animation may have hidden.
       ...
       Also hide the clone element. Completion callbacks do not fire on
       interruption, so clean up the clone here. */
    if (sourceCardEl) {
   ...
   * Open phase
   *
   * Capture the source thumbnail's bounding rect. Load the
   * full-resolution image into the modal. Let the browser lay it out.
   * Animate a clone from the source position to the target position.
   * The backdrop fades in with a slight stagger. The expanding clone
   * looks like it leads the transition.
   */
  ...
  const open = (imgSrc, imgAlt, sourceEl) =>
    new Promise((resolve) => {
      ...
      /* When the source thumbnail lives inside a .polaroid-frame, use the
         frame's bounding rect instead of the bare <img> rect. On small
         screens the frame padding is large. A clone sized to the image
         rect would visibly shrink inside its own padding. That mismatch
         is the main source of jitter on mobile.
         freezeRectFromEl corrects for the card's CSS rotation. The clone
         lands on the exact visual footprint, not the axis-aligned
         bounding box. */
      ...
      const startMorph = () => {
        /* Use the polaroid frame's bounding rect as the morph target.
           The expanding clone lands exactly where the polaroid frame
           appears. The framed look stays consistent through the
           transition. */
        ...
        /* Make the morph clone look like the modal polaroid frame. The
           clone-to-real swap at the end is then invisible. Without these
           styles, the clone is a bare full-bleed image. The real frame
           has white padding and a shadow. The mismatch snaps visibly when
           onOpenComplete hides the clone and shows the frame.
           Padding comes from the live computed style. It stays in sync
           with responsive media queries on small screens.
           When modalPolaroidFrame is null, fall back to empty padding
           and no shadow. */
      ...
      /* Apple Design §1 (Response): respond on pointer-down, not after
         ...
         starts with the already-loaded thumbnail src. It swaps to the
         full-res version when that arrives. The user sees instant motion
         ...
         full-res is almost always ready. If it is not, onOpenComplete
         handles the swap without a visible hiccup. */

      /* The thumbnail src is already loaded. It is instant. */
      ...
      const decodeState = fullResDecodeState.get(imgSrc);
      const fullResReady =
        decodeState === "decoded" ||
      ...
      if (fullResReady) {
        /* The bitmap is ready to paint without a decode (preloader warmed
           it). Or this image was never preloaded and is simply cached.
           Use the full-res directly. */
        ensureClone();
        morphCloneImg.src = imgSrc;
        startMorph();
      } else {
        /* Full-res is still loading or still undecoded. Morph with the
           already-decoded thumbnail now. The modal reveals the full-res
           when the animation completes. Never decode inside the animation
           window. (For a cached-but-undecoded image, the 'load' event
           already fired. So swapCloneToFullRes never runs. That is
           correct. The clone keeps the crisp thumbnail for the whole
           morph.) */
        ensureClone();
   ...
   * current modal image position. It animates back to wherever the
   * thumbnail is on screen at this moment. Use the same ease-out-quint
   * curve and distance-aware duration as the open. The motion feels
  ...
  const close = (sourceEl) =>
    new Promise((resolve) => {
      ...
      /* Prefer the element passed by the caller. Fall back to the
         element stored during open (the .polaroid-frame wrapper when
         the source image lives inside one). This lets the clone retrace
         to the frame-sized rect on all screen sizes.
         freezeRectFromEl corrects for CSS rotation. The clone's
         dimensions match the card's visual footprint, not its
      ...
      /* Style the clone to match the polaroid frame during the close
         ...
         content fades out.
         Padding and border-radius come from the live computed style.
      ...
      tl.to(morphClone, {
        /* Subtract the card's CSS translate offset from left/top.
           ...
           transform. This matches the card's rotate()+translate() chain.
